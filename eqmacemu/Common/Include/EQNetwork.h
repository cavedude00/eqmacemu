...
public:
	...
	void			QueuePacket(APPLAYER* app, bool ackreq = true);			// InQueuePush()
	...
	void			SetDataRate(float in_datarate)	{ datarate_tic = (sint32) (in_datarate * 100); datarate_sec = (int32) (in_datarate * 1000); dataflow = 0; datahigh = 0; } // conversion from kb/sec to byte/100ms, byte/1000ms
	float			GetDataRate()					{ return (float)datarate_sec / 1000; } // conversion back to kb/second
	inline bool		DataQueueFull()					{ return (dataflow > datarate_sec); }
	sint32			GetDataFlow()					{ return dataflow; }
