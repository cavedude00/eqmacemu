...
void LinkedListIterator<TYPE>::RemoveCurrent(bool DeleteData)
{
  ...
  safe_delete(current_element);//delete current_element;
...
//	cout << "ListElement<TYPE>::~ListElement()" << endl;
	...
	if (data != 0)
		safe_delete(data);//delete data;
	data = 0;
	if (next != 0)
	{
		safe_delete(next);//delete next;
		next = 0;
	}
}
...
LinkedList<TYPE>::~LinkedList()
{
	...
	if (first != 0)
	{
		safe_delete(first);//delete first;
	}
}
...
TYPE LinkedList<TYPE>::Pop() {
	...
	if (first) {
		...
		tmpdel->SetData(0);
		tmpdel->SetNext(0);
		safe_delete(tmpdel);//delete tmpdel;
	}
	return ret;
}
