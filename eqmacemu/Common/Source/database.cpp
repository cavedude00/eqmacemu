...
/*	char tmp[16];
	...
	if (GetVariable("startzone", tmp, 16)){
		uint32 zoneid = LoadZoneID(tmp);
		pp.current_zone = zoneid;
	}
	else {
		pp.current_zone = ZONE_ID;
	}*/
...
bool Database::GetSafePoints(char* short_name, float* safe_x, float* safe_y, float* safe_z, int8* minstatus, int8* minlevel, float* heading, bool GM)
{
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT safe_x, safe_y, safe_z, min_status, min_level, safe_heading FROM zone WHERE short_name='%s'", short_name), errbuf, &result)) {
		safe_delete_array(query);//delete[] query;
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			if(safe_x != 0)
...
bool Database::GetSafePointsByID(int32 zoneid, float* safe_x, float* safe_y, float* safe_z, int8* minstatus, int8* minlevel, float* heading, bool GM)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
	int buf_len = 256;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT safe_x, safe_y, safe_z, min_status, min_level, safe_heading FROM zone WHERE zoneidnumber='%i'", zoneid), errbuf, &result)) {
		safe_delete_array(query);//delete[] query;
		if(mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			if(safe_x != 0)
			...
			if (RunQuery(query, strlen(query), errbuf, &result))
			{
				...
				while(row = mysql_fetch_row(result))
				{
					...
					if (lengths[1] == sizeof(Item_Struct))
					{
						...
						if(item_array[atoi(row[0])]->type == 1)
						{
...
//				item->common.normal.click_effect_type = 3; // items with effect 
				...
				if(item->common.charges > 0 && i->IsConsumable()) {
					item->common.normal.stackable = 1;
				}				 
				...
				if(item->common.clicktype != 0 &&  item->common.charges > 0) {
					...
					item->common.normal.stackable = (uint8)atoi(row[ItemField::maxcharges]);
				}
...
//				item->common.container.BagType = (uint8)atoi(row[ItemField::bagtype]);
				...
				item->container.weightReduction = (uint8)atoi(row[ItemField::bagwr]);
			}
...
int16 Database::LoadZoneID(const char* zone)
{
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT zoneidnumber FROM zone WHERE short_name = '%s'", zone), errbuf, &result))
	{
		safe_delete_array(query);
		while(row = mysql_fetch_row(result))
		{
...
bool Database::GetZoneShortName(int16 zoneID, char* name)
{
	...
	if(RunQuery(query, MakeAnyLenString(&query, "SELECT short_name FROM zone WHERE zoneidnumber = %i", zoneID), errbuf, &result))
	{
		safe_delete_array(query);
		while(row = mysql_fetch_row(result))
		{
...
char* Database::GetZoneName(int32 zoneID, bool ErrorUnknown) {
	if (zonename_array == 0) {
		if (ErrorUnknown)
			return "UNKNOWN";
		else
...
void Database::GetZoneCFG(int16 zoneID, int8* sky, int8* fog_red, int8* fog_green, int8* fog_blue, float* fog_minclip, float* fog_maxclip, float* minclip, float* maxclip, uint8* ztype, float* safe_x, float* safe_y, float* safe_z, float* underworld, char** zlong_name)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
    MYSQL_RES *result;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT sky, fog_red, fog_green, fog_blue, fog_minclip, fog_maxclip, minclip, maxclip, ztype, safe_x, safe_y, safe_z, underworld, long_name from zone where zoneidnumber=%i",zoneID), errbuf, &result)) 
	{
		safe_delete_array(query);//delete[] query;
		if (mysql_num_rows(result) == 1)
		{
			...
			*underworld = atof(row[12]);
			//*walkspeed = atof(row[13]);
			if (zlong_name != 0) {
				*zlong_name = strcpy(new char[strlen(row[13])+1], row[13]);
			}
			safe_delete_array(query);//delete[] query;
			mysql_free_result(result);
			return;
