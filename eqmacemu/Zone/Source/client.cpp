	  ...
	  )
{
	...
	debugMe = true; // Harakiri default false, YL debug is annoying =p
	myEyeOfZomm = NULL;
	mySaveChance = 0;
	feared = false;
...
int16 Client::AutoPutItemInInventory(Item_Struct* item, sint8 charges,int tradeslot){
	...
	if(item->common.normal.stackable==1){ // if item can stack
		...
		for (i = 22; i <= 29; i++)
		{
			...
			if (bag != 0 && bag->type == 0x01) { // make sure it's a container
				...
				for (uint8 j = 0; j < bag->container.BagSlots; j++)
				{
	...
	if (!ForBag) {
		...
		for (i=22; i<=29; i++) {
			...
			if (item != 0 && item->type == 0x01) { // make sure it's a container
				for (int k=0; k<item->container.BagSlots; k++) {
...
void Client::SendStationItem(int32 oldPlayerID){
	...
	{	
		...
		if(this->stationItems[i]!=0){
			...
			{
				if(item->common.normal.stackable==1){//Tazadar load the number in each stack for inventory (not container)
...
void Client::SendTranslocateConfirmation(Mob* caster, Spell* spell)
{
	...
	if(pendingTranslocateSpellID == 1422 /*Translocate*/ || pendingTranslocateSpellID ==  1334 /*GroupTranslocate*/)
	{
		...
		const char* zone_name = Database::Instance()->GetZoneName(this->GetPlayerProfilePtr()->bind_point_zone);
