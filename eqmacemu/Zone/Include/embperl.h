...
// Harakiri solve issues that perl overrides SOCKET function of winsock
// without this, the following will happend when the embperl.h  is loaded into
// a cpp where socket functions happend (worldserver, client.cpp) :
// there will just be an access violation because socket is used from perl
// EQEMU has its socket handling code seperate, but we have it all over
// this workaround below works fine so far
