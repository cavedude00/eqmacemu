...
public:
	...
	bool	GetSafePointsByID(int32 zoneid, float* safe_x = 0, float* safe_y = 0, float* safe_z = 0, int8* minstatus = 0, int8* minlevel = 0, float* heading = 0, bool GM = false);
	bool	LoadItems_Old();
	...
	bool	LoadZoneNames();
	char*	GetZoneName(int32 zoneID, bool ErrorUnknown = false);
