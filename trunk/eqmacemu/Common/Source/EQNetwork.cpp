...
void EQNetworkConnection::QueuePacket(APPLAYER* app, bool ackreq) {
	if (!app) {
...
//		SetState(EQNC_Error);
	...
	while (iterator.MoreElements() && (!DataQueueFull())) {
		pack = iterator.GetData();
		if (((pack->LastSent + 1000) <= Timer::GetCurrentTime()) || (datahigh < 5 && (pack->LastSent + 500) <= Timer::GetCurrentTime())) {
#if EQN_DEBUG >= 8
...
//		pack->dwOpCode = 0xFFFF;
	...
	/************ PM STATE = ACTIVE ************/
		for (int i=0; i<=frags; i++) {
			...
			if (i==0) // If this will be a fragmented packet, only include ackseq in first fragment
...
/*	if (size < 10) {	// Agz: Adding checks for illegal packets, so we dont read beyond the buffer
    ...
    if (HDR.b4_Unknown) {
			...
			cout << "HDR.b4: 0x" << hex << (int) data[o] << endl;
		#endif
		o++; // One unknown byte
    }
	/*
	...
	*/
    if (HDR.b5_Unknown) {
		#if EQN_DEBUG_Unknown >= 5
			cout << "HDR.b5: 0x" << hex << setw(2) << setfill('0') << *((int16*)&data[o]) << dec << endl;
		#endif
        o += 2; // 2 unknown bytes
    }
	if (HDR.b6_Unknown) {
		#if EQN_DEBUG_Unknown >= 5
			cout << "HDR.b6: 0x" << hex << setw(4) << setfill('0') << *((int32*)&data[o]) << dec << endl;
		#endif
		o += 4; // 4 unknown bytes
	}
	if (HDR.b7_Unknown) {
		#if EQN_DEBUG_Unknown >= 5
			cout << "HDR.b7: 0x" << hex << setw(4) << setfill('0') << *((int32*)&data[o]) << dec << " 0x" << hex << setw(4) << setfill('0') << *((int32*)&data[o+4]) << dec << endl;
		#endif
		o += 8; // 8 unknown bytes
	}
...
int32 EQNetworkPacket::ReturnPacket(uchar** data) {
	*data = new uchar[dwExtraSize + 37]; // Quag: 37 is the max header + opcode + crc32 + unknowns size
	uchar* buf = *data;
	int32 o = 4;
	...
	*((int16*)&buf[2]) = ntohs(netcon->SACK.dwGSQ++);
	...
	memcpy(buf, &HDR, 2);
	*((int32*)&buf[o]) = GenerateCRC(0, o, buf);
	o += 4;
	return o;
}
