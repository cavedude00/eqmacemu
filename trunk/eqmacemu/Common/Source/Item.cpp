...
bool ItemInst::IsStackable() const
{
	if(IsType(ItemClassCommon)) {
		return m_item->common.normal.stackable;
	}
...
bool Inventory::HasSpaceForItem(const Item_Struct *ItemToTry, uint8 Quantity) {
	...
	if(ItemToTry->common.normal.stackable) {
		...
		for(sint16 i = 22; i <= 29; i++) {
			...
			if (InvItem && InvItem->IsType(ItemClassContainer)) {
				...
				int8 BagSize=InvItem->GetItem()->container.BagSlots;
...
bool Inventory::CanItemFitInContainer(const Item_Struct *ItemToTry, const Item_Struct *Container) {
	...
	if(ItemToTry->size > Container->container.BagSize) return false;

//	if((Container->container.BagType == bagTypeQuiver) && (ItemToTry->type != ItemTypeArrow)) return false;

//	if((Container->container.BagType == bagTypeBandolier) && (ItemToTry->type != ItemTypeThrowingv2)) return false;
