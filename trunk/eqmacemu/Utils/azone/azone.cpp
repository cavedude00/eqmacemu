...
QTBuilder::~QTBuilder() {
	if(_root != NULL)
		safe_delete(_root);//delete _root;
	_root = NULL;
	/*	if(vertexBlock != NULL)
	delete vertexBlock;
	vertexBlock = NULL;*/
	if(faceBlock != NULL)
		safe_delete(faceBlock);//delete faceBlock;
	faceBlock = NULL;
}
...
bool QTBuilder::writeMap(const char *file) {
	...
	fclose(out);
	safe_delete_array(nodes);//delete[] nodes;
	safe_delete_array(facelist);//delete[] facelist;
...
void QTNode::clearNodes() {
	if(node1 != NULL)
		safe_delete(node1);//delete node1;
	if(node2 != NULL)
		safe_delete(node2);//delete node2;
	if(node3 != NULL)
		safe_delete(node3);//delete node3;
	if(node4 != NULL)
		safe_delete(node4);//delete node4;
	//node1 = NULL;
	//node2 = NULL;
	//node3 = NULL;
	//node4 = NULL;
...
safe_delete_array(fblock);//delete[] fblock;
