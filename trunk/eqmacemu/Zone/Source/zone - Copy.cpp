...
bool Zone::Bootup(int32 zoneid)
{
	...
	numclients = 0;
	zone = new Zone(zoneid, net.GetZoneAddress(), net.GetZonePort());
	...
	ZoneLoaded = true;
	worldserver.SetZone(zoneid);
	char bfr[64];memset(bfr, 0, sizeof(bfr));
	sprintf(bfr, "EQC: -Running %i:%i", zoneid, net.GetZonePort());
	cout << "-----------" << endl;
	EQC::Common::PrintF(CP_ZONESERVER, "Zone server '%i' now listening on port %i\n", zoneid, net.GetZonePort());
	cout << "-----------" << endl;
	EQC::Common::PrintF(CP_ZONESERVER, "Loading NPC's for '%i'...\n",zoneid);
//	char* zone_name = Database::Instance()->GetZoneName(zoneid);
...
Zone::Zone(int32 in_zone_id, char* in_address, int16 in_port)
{
	bulkOnly = true;
	...
	Database::Instance()->GetZoneLongName(zoneid, &long_name, file_name, &psafe_x, &psafe_y, &psafe_z);
	...
	char name[16] = "";
	Database::Instance()->GetZoneShortName(zoneid,name);

	if (short_name == 0)
	{
		short_name = strcpy(new char[7], "UNKNOWN");
	}
	...
	SetSafeZ(-1);
	SetSafeHeading(-1);
	if(Database::Instance()->GetSafePointsByID(zoneid, &safeX, &safeY, &safeZ, 0, 0, &safeHeading) == false)
		EQC::Common::Log(EQCLog::Status,CP_ZONESERVER, "ERROR loading zone safe points for: %i",zoneid);
...
Zone::~Zone()
{
	...
	if (worldserver.Connected()) {
		...
		scl->remove = true;
		strcpy(Database::Instance()->GetZoneName(scl->zone), short_name);
