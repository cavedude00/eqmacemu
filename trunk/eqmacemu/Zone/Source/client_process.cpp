...
void Client::SendInventoryItems() 
{
	...
	/*	if(item && item->norent==0){ //If we have a no rent item and loged of for more than 30 mins we remove item
		...
		if (item) 
		{
			...
			if(item->common.normal.stackable==1){//Tazadar load the number in each stack for bag (not container)
...
void Client::ProcessOP_Forage(APPLAYER* pApp){
	uint32 food_id = ForageItem(Database::Instance()->GetZoneName(pp.current_zone), GetSkill(FORAGE));
...
void Client::Process_ClientConnection2(APPLAYER *app)
{
	...
	// pp.discplineAvailable = 1; cavedude - not sure where this is in our pp
